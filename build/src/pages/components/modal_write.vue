      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form id="upload-file" method="post" class="form-horizontal" @submit.prevent="writeSubmit" enctype="multipart/form-data">
              ...
              <div class="form-group">
                ...
                <div class="form-group">
                  <input type="file" id="file" name="file" ref="file1" accept=".jpg, .jpeg, .gif, .png" class="custom-file-input" v-on:change="previewFiles($event)">
                </div>
              </div>
              <div v-for="(link, index) in list.links">
                <div class="form-group">
                  ...
                  <div v-show="index === 0" class="col-sm-1">
                    <button type="button" v-on:click="addNewApartment()" class="btn btn-block btn-success">+</button>
                  </div>
                  <div v-show="index !== 0" class="col-sm-1">
...
<script>
  ...
  export default {
    data() {
      ...
      updated () {
          console.log('write 모달  updated');
      },
    computed: {
      ...mapState('account', ['status']),
    },
    methods: {
      ...mapActions('lists', ['listRegister']),
      addNewApartment : function () {
        if (this.list.links.length < 10 && this.list.links[0] !== '' && this.list.links.find('')) {
